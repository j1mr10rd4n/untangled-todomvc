    ...
    (let [{:keys [db/id item/label item/complete ui/editing]} (om/props this)
          ...
          submit-edit (fn [evt]
                        (if-let [trimmed-text (trim-text (.. evt -target -value))]
                          (do
                            (om/transact! this `[(mut/edit-todo ~{:id id :text trimmed-text})])
  ...
  (render [this]
    (let [{:keys [list/items list/filter list/title db/id]} (om/props this)
          ...
          filtered-todos (case filter
                           ...
                           items)
          delete-item (fn [item-id] (om/transact! this `[(mut/delete-todo ~{:id item-id})]))
          check (fn [item-id] (om/transact! this `[(mut/check-todo ~{:id item-id})]))
          uncheck (fn [item-id] (om/transact! this `[(mut/uncheck-todo ~{:id item-id})]))]
              ...
              (dom/section #js {:className "main"}
                (dom/input #js {:className "toggle-all"
                                ...
                                :onClick   (fn [] (if all-completed?
                                                    (om/transact! this `[(mut/uncheck-all-todos)])
                                                    (om/transact! this `[(mut/check-all-todos)])))})
    ...
    (letfn [(add-item [evt]
              (when (is-enter? evt)
                (when-let [trimmed-text (trim-text (.. evt -target -value))]
                  (om/transact! this `[(mut/add-todo ~{:id (om/tempid) :text trimmed-text})])
      ...
      (dom/footer #js {:className "footer"}
        ...
        (when (pos? num-completed)
          (dom/button #js {:className "clear-completed"
                           :onClick   #(om/transact! this `[(mut/clear-complete-todos)])} (tr "Clear Completed"))))))
          ...
          (if support-visible
            (dom/div #js {}
              ...
              (dom/button #js {:onClick (fn []
                                          (om/transact! this `[(mut/send-support-request {:comment ~comment})
                                                               (mut/toggle-support-request)])
                                          (om/update-state! this assoc :comment ""))}
                (tr "Send Request")))
            (dom/button #js {:onClick #(om/transact! this `[(mut/toggle-support-request)])} (tr "Help!"))))
